...
import sys
import subprocess
from test._runner import check, section, summary, reset, SONG_ID, PLAYLIST_ID, ALBUM_ID, PROJECT_ROOT
...
r = run(["playlist", str(PLAYLIST_ID), "-d", "--limit", "1", "-o", "/tmp/vnemd_cli_test"])
check("playlist -d", r.returncode == 0)

r = run(["album", str(ALBUM_ID), "-d", "--limit", "1", "-o", "/tmp/vnemd_cli_test"])
