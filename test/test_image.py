...
def test_quality_0_same_as_input():
    original = _make_jpeg()
    d0, _ = process_cover(original, "http://test.vanadiry.com/cover.jpg", "0")
    assert d0 == original


def test_quality_0_mime_jpeg():
    original = _make_jpeg()
    _, m0 = process_cover(original, "http://test.vanadiry.com/cover.jpg", "0")
    assert m0 == "image/jpeg"


def test_quality_1_mime_jpeg():
    original = _make_jpeg()
    _, m1 = process_cover(original, "http://test.vanadiry.com/cover.jpg", "1")
    assert m1 == "image/jpeg"


def test_quality_1_smaller():
    original = _make_jpeg()
    d1, _ = process_cover(original, "http://test.vanadiry.com/cover.jpg", "1")
...
def test_small_image_not_larger():
    ...
    small = buf.getvalue()
    d_small, _ = process_cover(small, "http://test.vanadiry.com/small.png", "1")
...
def test_rgba_to_jpeg():
    ...
    _, m = process_cover(buf.getvalue(), "http://test.vanadiry.com/cover.png", "1")
    assert m == "image/jpeg"
...
def test_rgba_valid():
    buf = io.BytesIO()
    PILImage.new("RGBA", (600, 600), (255, 0, 0, 128)).save(buf, "PNG")
    d, _ = process_cover(buf.getvalue(), "http://test.vanadiry.com/cover.png", "1")
